...
def evaluate(data, test_data, config):
    ...
    if dataset_split == r'C:\Users\HP\Downloads\screatchdementiapycharm\models\testing_silence\3_c\compare':  # compare features need to be projected
        ...
        for i in range(config.n_folds):
            ...
            sc = load(open(os.path.join(config.model_dir, 'compare/scaler{}.pkl'.format(fold + 1)), 'rb'))
            pca = load(open(os.path.join(config.model_dir, 'compare/pca{}.pkl'.format(fold + 1)), 'rb'))
            ...
            if len(model_types) == 1:
                ...
                if m == 'compare':
                    print('Fold {}'.format(fold + 1))
                    print('Train')
                    train_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], compare_train, y_train,
                                                             config, fold=fold)
                    print('Val')
                    val_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], compare_val, y_val, config,
                                                           fold=fold)
                    print('Test')
                    test_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], compare_test, y_test,
                                                            config, fold=fold)
                else:
                    print('Fold {}'.format(fold + 1))
                    print('Train')
                    train_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], data[m][:n_train],
                                                             y_train, config, fold=fold)
                    print('Val')
                    val_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], data[m][n_train:], y_val,
                                                           config, fold=fold)
                    print('Test')
                    test_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], test_data[m], y_test,
                                                            config, fold=fold)
            else:
                models = []
                features = []
                ...
                for m in model_types:
                    if m == 'compare':
                        ...
                        features.append(data[m][n_train:])
                val_accuracy, _, _ = get_ensemble_accuracy(task, models, features, y_val, config,
                                                           learnt_voter=learnt_voter, fold=fold)
                ...
                for m in model_types:
                    if m == 'compare':
                        ...
                        features.append(test_data[m])
                test_accuracy, _, average_results = get_ensemble_accuracy(task, models, features, y_test, config,
                                                                          learnt_voter=learnt_voter, fold=fold,
                                                                          plot=config.plot)
            ...
            if len(model_types) == 1:
                ...
                if m == 'compare':
                    ...
                    train_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], compare_train, y_train,
                                                             fold=fold)
                    print('Val')
                    val_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], compare_val, y_val,
                                                           fold=fold)

                else:
                    print('Fold {}'.format(fold + 1))
                    print('Train')
                    train_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], data[m][train_index],
                                                             y_train, fold=fold)
                    print('Val')
                    val_accuracy = get_individual_accuracy(task, saved_model_types[m][fold], data[m][val_index], y_val,
                                                           fold=fold)
                ...
                for m in model_types:
                    if m == 'compare':
                        ...
                        features.append(data[m][val_index])
                val_accuracy, _ = get_ensemble_accuracy(task, models, features, y_val, config,
                                                        learnt_voter=learnt_voter, fold=fold)
...
def get_individual_accuracy(task, model, feature, y, config, fold=None):
    if task == 'classification':
        ...
        accuracy = accuracy_score(np.argmax(y, axis=-1), preds)
        report = precision_recall_fscore_support(np.argmax(y, axis=-1), preds, average='binary')
        print('precision: {:.3f}, recall: {:.3f}, f1_score: {:.3f}, accuracy: {:.3f}'.format(report[0], report[1],
                                                                                             report[2], accuracy))
        return accuracy
...
def evaluate(data, test_data, config):
    ...
    if dataset == r'C:\Users\HP\Downloads\datset_dir':
        # ...
        ...
        # Print RMSE score
        print('Train RMSE: {:.3f}'.format(train_accuracy))
